...
class LloydsAlternateModulusElevenCheck {
    fun check(params: ModulusCheckParams, weightRow: WeightRow): Boolean =
        if (WeightRow.isExceptionTwoAndNine(params.firstWeightRow, params.secondWeightRow)) {
            checkLloydsAccount(params, weightRow)
        } else {
            false
        }

    private fun checkLloydsAccount(params: ModulusCheckParams, weightRow: WeightRow): Boolean {
